...
pub unsafe fn reset_handler() {
    ...
    for (proc_num, proc_) in PROCESSES.as_ref().iter().enumerate() {
        match proc_ {
            Some(p) => load_process_hook(
                proc_num as u32,
                p.get_process_name(),
                p.flash_non_protected_start() as u32,
            ),
            None => (),
        }
    }
...
// This function's sole purpose is to allow GDB to observe process loading
// and load debug symbols from the `.elf` that corresponds to that
// process.
#[no_mangle]
